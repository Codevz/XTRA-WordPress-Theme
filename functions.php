...
if ( ! class_exists( 'Codevz_Core_Theme' ) ) {
	...
	class Codevz_Core_Theme {
			...
			if ( ! isset( $_POST['vc_inline'] ) ) {
				...
				if ( function_exists( 'is_woocommerce' ) ) {
					...
					if ( $color ) {
						...
						$css .= '.woocommerce div.product form.cart .variations .codevz-variations input[type="radio"]:checked + label{outline-color:' . esc_html( $color ) . ';color:' . esc_html( $color ) . '}';
		...
		public static function is_free( $is_free = false ) {
			...
			return self::$plugin ? Codevz_Plus::is_free() : true;
			...
			if ( $is_404 ) {
				...
				if ( $page_404 ) {
					...
					if ( self::option( 'disable_search_404' ) ) {
						...
						echo '<form class="search_404" method="get" action="' . esc_url( $url ) . '">
		...
		public static function breadcrumbs_array() {
			...
			$bc = [];
			$bc[] = [ 'home' => true, 'title' => '<i class="fa fa-home cz_breadcrumbs_home" aria-hidden="true"></i>', 'link' => trailingslashit( get_home_url() ) ];
